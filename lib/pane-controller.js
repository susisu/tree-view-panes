...
export default class PaneController {
  ...
  restoreItemControllers() {
    if (this.itemCtrlers.length > 0) {
      // remove old item-views and item-controllers
      this.view.removeAllItemViews();
  ...
  findItemControllerIndexOf(item) {
    for (let index = 0; index < this.itemCtrlers.length; index++) {
      if (this.itemCtrlers[index].item === item) {
        return index;
      }
    }
    return -1;
  ...
  addItemControllerOf(item, index) {
    ...
    if (typeof this.pane.getPendingItem === 'function' && this.pane.getPendingItem() === item) {
  ...
  removeItemControllerOf(item, index) {
    ...
    if (!itemCtrler || itemCtrler.item !== item) {
      this.restoreItemControllers();
      return;
    }
  ...
  setActiveItem(item) {
    ...
    // item can be empty when there are no items
    if (!item) {
      return;
    }
    const index = this.findItemControllerIndexOf(item);
    if (index < 0) {
      this.restoreItemControllers();
      return;
    }
  ...
  unsetPendingItem(item) {
    ...
    this.itemCtrlers[index].setPending(false);
  }
