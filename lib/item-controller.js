...
export default class ItemController {
  constructor(parent, item, index) {
    ...
    this.view.setShowTooltip(atom.config.get(`${CONFIG_PREFIX}showTooltips`));
    ...
    if (typeof this.item.onDidChangeTitle === 'function') {
      this.subscriptions.add(this.item.onDidChangeTitle(() => {
        this.parent.parent.updateItemTitles();
      }));
    }
    if (typeof this.item.onDidChangePath === 'function') {
      this.subscriptions.add(this.item.onDidChangePath(() => {
        this.path     = this.getItemPath();
        this.realpath = this.path;
        this.updatePath();
        this.updateIcon();
    ...
    if (typeof this.item.onDidChangeModified === 'function') {
      this.subscriptions.add(this.item.onDidChangeModified(() => {
        this.updateModified();
      }));
    }
    ...
    this.subscriptions.add(atom.config.observe(
      `${CONFIG_PREFIX}showTooltips`,
      showTooltips => {
        this.view.setShowTooltip(showTooltips);
      }
    ));
    ...
    this.onDblClick = () => {
      if (typeof this.item.terminatePendingState === 'function') {
    ...
    this.onDrop = event => {
      ...
      // find controllers
      const fromPaneCtrler = this.parent.parent.getPaneControllerAt(fromPaneIndex);
      if (!fromPaneCtrler) {
        return;
      }
      ...
      const toIndex = (fromPaneCtrler === this.parent && itemCtrler.index <= this.index)
        ? this.index
        : this.index + 1;
    ...
    this.view.element.addEventListener('dblclick', this.onDblClick);
    ...
    this.view.closeButtonElement.addEventListener('click', this.onCloseButtonClick);
    this.subscriptions.add(new Disposable(() => {
      this.view.element.removeEventListener('click', this.onClick);
      this.view.element.removeEventListener('dblclick', this.onDblClick);
      ...
      this.view.closeButtonElement.removeEventListener('click', this.onCloseButtonClick);
    }));
  ...
  updateTitle() {
    const title = typeof this.item.getLongTitle === 'function' ? this.item.getLongTitle()
  ...
  setTooltipPlacement(tooltipPlacement) {
    this.view.setTooltipPlacement(tooltipPlacement);
  }
  ...
  copyItem() {
    ...
    else if (this.parent.pane.deserializerManager && typeof this.item.serialize === 'function') {
      return [this.parent.pane.deserializerManager.deserialize(this.item.serialize())];
    }
    else {
      return [];
    }
