...
export default class ItemController {
  ...
  updatePath() {
    const filePath = this.getItemPath();
    if (filePath) {
      this.view.setName(path.basename(filePath));
      this.view.setPath(filePath);
    }
    else {
  ...
  updateIcon() {
    if (typeof this.item.getIconName === 'function') {
      ...
      const filePath = this.getItemPath();
      if (filePath) {
        const iconClass = FileIcons.getIconClassForPath(filePath);
