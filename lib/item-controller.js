...
export default class ItemController {
  constructor(parent, item, index) {
    ...
    this.onDragStart = event => {
      ...
      copyStyle(this.view.nameElement, dragImage);
...
function copyStyle(fromElement, toElement) {
  const style = window.getComputedStyle(fromElement);
  const wordRegex = /\-([a-z])/g;
  const headRegex = /^[A-Z]/;
  for (const prop of style) {
    const key = prop.replace(wordRegex, (_, c) => c.toUpperCase())
      .replace(headRegex, c => c.toLowerCase());
    toElement.style[key] = style.getPropertyValue(prop);
  }
}
