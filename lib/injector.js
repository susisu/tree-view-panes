...
export default class Injector {
  constructor(state) {
    ...
    this.subscriptions.add(atom.workspace.onDidAddPaneItem(event => {
      if (this.treeViewPkg && event.item === this.treeViewPkg.mainModule.treeView) {
        this.setTreeViewItem(this.treeViewPkg.mainModule.treeView);
      }
    }));
  ...
  injectView() {
    if (this.treeViewItem && !this.injected) {
      ...
      this.parentElement.replaceChild(this.wrapperElement, this.originalElement);
      this.wrapperElement.appendChild(this.controller.view.element);
      this.wrapperElement.appendChild(this.originalElement);
      // override getPreferredWidth
      this.treeViewItem.getPreferredWidth = () => {
  ...
  ejectView() {
    if (this.treeViewItem && this.injected) {
      ...
      this.parentElement.replaceChild(this.originalElement, this.wrapperElement);
      this.parentElement   = null;
      this.originalElement = null;
      this.wrapperElement  = null;
      // remove overridden getPreferredWidth
